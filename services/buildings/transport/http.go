...
func NewService(svcEndpoints Endpoints, logger *logging.Logger) http.Handler {
	...
	r.Methods("POST").Path("/buildings/getBuildingsByFilter").Handler(kithttp.NewServer(
		svcEndpoints.GetBuildingsByFilter,
		decodeBuildingsByFilterRequest,
...
func decodeBuildingsByFilterRequest(_ context.Context, r *http.Request) (request interface{}, err error) {

	var req model.GetBuildingsByFilterRequest
